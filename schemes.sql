...
CREATE TABLE IF NOT EXISTS character_extras (
    last_loss_date INT, last_kill_date INT,
    avg_attackers NUMERIC,
    covert_prob NUMERIC, normal_prob NUMERIC,
    last_cov_ship INT, last_norm_ship INT,
    abyssal_losses INT,
    last_update TEXT);

CREATE TABLE IF NOT EXISTS cyno_kills (
    killmail_id INT,
    ship_id INT,
    is_covert BOOLEAN,
    is_normal BOOLEAN);
